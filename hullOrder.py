...
class hullOrderer:
    def __init__(self):
        self.ordered = np.zeros((4,2), dtype=np.float32)
        self.ordered3 = np.zeros((3,2), dtype=np.float32)
        self.temp = np.ones((4,2), dtype=np.float32)
    ...
    def organizer(self, imagePoints):
        if(len(imagePoints) == 4):
            ...
            # respahing to (4,2) from (4,1,2)
            imagePoints = np.reshape(imagePoints, (3,2))
            # taking the values of x and y
            x_point = imagePoints[imagePoints[:, 0].argsort()] # sorting wth x values
            y_point = imagePoints[imagePoints[:, 1].argsort()] # sorting wth y values
            print("x axis is ", x_point)
            ...
            self.ordered3[0] = x_point[0] # bottom left point

            if ((x_point[2] == y_point[2])[0] & (x_point[2] == y_point[2])[1]) or ((x_point[2] == y_point[1])[0] & (x_point[2] == y_point[1])[1]):
                self.ordered3[2] = x_point[2] # bottom right point
                self.ordered3[1] = x_point[1] # upper right point
            else:
                self.ordered3[2] = x_point[1] # bottom right point
                self.ordered3[1] = x_point[2] # upper right point

            return self.ordered3
        else:
            return self.temp